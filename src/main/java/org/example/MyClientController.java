...
public class MyClientController {
    ...
    private int port = 80;
    private String serverURL = "localhost"; //PUT Your Server IP
    ...
    public void login() {
        ...
        try {
            ...
            System.out.println("Login response : \n" + response); //Before submission, apply comment out.
    ...
    public void getProducts() {
        ...
        try {
            ...
            if (response.indexOf("HTTP/") != -1) {
                if (response.indexOf("200 OK") != -1) {
                    String[] TotalResponse = response.split("\r\n\r\n");
                    String ResponseBody = TotalResponse[1];
                    String[] items = ResponseBody.split("\r\n");
                    v.model.setRowCount(0);
                    for(int i=0; i<items.length; i++) {
                        Product p = gson.fromJson(items[i], Product.class);
                        String[] arr = new String[4];
                        arr[0] = p.getOrderId().toString();
                        arr[1] = p.getName();
                        arr[2] = p.getStatus();
                        arr[3] = p.getCreatedAt();
                        v.model.addRow(arr);
                    }
                }
            }
